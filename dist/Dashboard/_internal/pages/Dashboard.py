...
class DashboardPage(tk.Frame):
    def __init__(self, parent, colors):
        ...
        # Use the most recent scan time across both sources for "Last Scan"
        ...
        mal_pct            = float(mal_summary.get("attack_percentage", 0) or 0)
        ...
        # Malware meter: show infected out of total for last scan
        # Falls back to quarantined total if no scan summary available yet
        if mal_total > 0:
        ...
        self._create_meter_card(
            ...
            icon="🦠",
            amount_used=mal_meter_used,
            amount_total=mal_meter_total,
            unit=mal_unit,
            threat_level="attack" if mal_infected > 0 else ("suspicious" if mal_high_entropy > 0 else "none"),
            subtext=mal_subtext,
        )
        ...
        # Right panel — scrollable events (merged from both sources)
        ...
        # Merge and sort events from both sources newest-first
    ...
    def create_event_row(self, parent, title, detail, when, source=None):
        ...
        # Small source badge (🌐 network / 🦠 malware)
        if source == "network":
            badge_text  = "🌐"
            badge_color = self.colors.get("orange_secondary", "#ff8c42")
        elif source == "malware":
            badge_text  = "🦠"
            badge_color = self.colors.get("danger", "#f44336")
        else:
            badge_text  = ""
